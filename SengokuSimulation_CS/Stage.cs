...
namespace SengokuSimulation_CS
{
    ...
    internal class Stage
    {
        ...
        public bool IsHonnojiEvent()
        {
            // 1592年で二条城のオーナーは織田信長?
            return m_year == 1592
                && GetCastleOwner(CastleId.NIJO) == LordId.ODA;
        }
        // ターン実行
        public void ExecTurn(int turn)
        {
            ...
            PrintTurnOrder(turn);
            Utility.Printf("{0}けの　{1}の　ひょうじょうちゅう…\n"
        ...
        protected void SiegeBattle(LordId offenseLord, int offenseTroopCount, CastleId defenseCastle)
        {
            ...
            while (true)
            {
                Utility.Printf("{0}ぐん（{1,4}にん）　Ｘ　{2}ぐん（{3,4}にん）\n"
            ...
            if (defenseTroopCount <= 0)
            {
                ...
                Utility.Printf("{0}は　 {1}けの　ものとなります\n"
                    , GetCastleName(defenseCastle)
                    , GetLordFamilyName(offenseLord)
                );
                Utility.PrintOut();
        ...
        {
            if (id == LordId.ODA && m_isDoneHonnojiEvent)
            {
                id = LordId.HASHIBA;
            }
            return id;
